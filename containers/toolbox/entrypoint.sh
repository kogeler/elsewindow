...
# Elsewindow client-container entry point.
#
...
if (( status != 0 )) && [[ "${BOX_EXPORT_ON_SUCCESS:-0}" == 1 ]]; then
    /usr/local/libexec/container-payload create --root=/work/src --allow-empty
    exit "$status"
fi
...
/usr/local/libexec/container-payload create --root=/work/src -- "${exported[@]}"
exit "$status"

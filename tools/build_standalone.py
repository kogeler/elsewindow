...
"""Build and describe one native Linux one-file Elsewindow executable."""
...
def standalone_name(
    ...
    return f"elsewindow-linux-{architecture}"
...
def _source_digest(root: Path) -> str:
    ...
    paths = [
        ...
        *sorted(
            path
            for path in (root / "elsewindow").rglob("*")
...
def build(
    *, root: Path, python: Path, output: Path, epoch: int, expected_arch: str | None
    ...
    command = [
        ...
        "ssh-wrapper",
        "--add-data",
        f"{root / '.version'}{separator}elsewindow",
        "--add-data",
        f"{root / 'elsewindow/live-cli.yml'}{separator}elsewindow",
        "--add-data",

...
"""Audit every frozen dependency audience against exact exceptions."""
...
DEFAULT_LOCKS = (
    DEFAULT_ROOT / "requirements.txt",
    DEFAULT_ROOT / "requirements-quality.txt",
    DEFAULT_ROOT / "requirements-test.txt",
...
def _run_pip_audit() -> frozenset[Finding]:
    """Strictly audit every complete hash-locked audience."""
    findings: set[Finding] = set()
    for lock in DEFAULT_LOCKS:
        ...
        if result.returncode not in (0, 1):
            ...
            raise AuditError(f"pip-audit failed for {lock.name}: {detail}")
        try:
            findings.update(_audit_findings(json.loads(result.stdout)))
        except json.JSONDecodeError as err:
            raise AuditError(
                f"pip-audit returned invalid JSON for {lock.name}: {err}"
            ) from err
    return frozenset(findings)
...
def _run(args: argparse.Namespace) -> None:
    ...
    actual = (
        _audit_findings(_load_json(args.report, source="pip-audit report"))
